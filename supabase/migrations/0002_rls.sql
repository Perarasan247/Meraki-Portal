...
create or replace function auth.jwt_branch_id() returns uuid
language sql stable as $$
...
create policy branches_self_read on branches
  for select using (id = auth.jwt_branch_id());
...
create policy profiles_branch_admin_read on profiles
  for select using (
    auth.jwt_role() = 'branch_admin' and branch_id = auth.jwt_branch_id()
  );
...
create policy enquiries_branch_all on enquiries
  for all using (branch_id = auth.jwt_branch_id())
  with check (branch_id = auth.jwt_branch_id());
...
create policy audit_log_branch_insert on audit_log
  for insert with check (branch_id = auth.jwt_branch_id());
